...
def SimplestOT_1OutOf2(sender, receiver, multi=False, verbose:bool = False):
    """
    ...
    calculate_public_keys(
        sender, receiver, generator, a, b, prime_p
    )  # We calculate A and B for the protocol
    ...
    if verbose:
        print(
            f"Welcome to the simplest 1-out-of-2 OT protocol simulator \n"
            f"Sender's messages: m0: {sender.m0 } m1: {sender.m1}\n"
            f"Receiver's choice: {receiver.choice_bit} \n"
        )
...
def oneOtOfNOT(sender, receiver, verbose=False):
    ...
    if verbose:
        print(
            f"message vector {sender.get_message_list()}\nThe receiver wants to observe the message at index { receiver.get_choice_bit()}\n"
            f"The keypairs are : {key_pairs}"
            f"The message received is: {sender.Y[receiver.placeHolder - 1] ^ receiver_xored_keys}"
        )
...
def VOLE(Alice: OLE_Sender, Bob: OLE_Receiver, q: int, verbose=False):
    """
    ...
    if verbose:
        print(
            f"result in binary: {OLE_y} = {VOLE_Reconstruction(reverse_binary_list(OLE_y))%q} modulo {q}"
        )
        print(
            f"expected result: b + x*a = {Bob.b} + {Alice.x} * {Bob.a} modulo {q} = {(Bob.b +( Alice.x*Bob.a))%q}"
        )
